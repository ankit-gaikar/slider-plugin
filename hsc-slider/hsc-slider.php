...
function hsc_slider_scripts(){
	...
	wp_enqueue_script( 'slick-js',  plugin_dir_url(__FILE__).'assets/slick.min.js');
...
function hsc_slider_shortcode( $atts){
	$atts = shortcode_atts(array( 'id' => '', 'name' => '', 'dots' => 'false', 'arrows' => 'true', 'infinte' => 'true', 'class' => 'hsc-slider', 'slides' => '1'), $atts);
	$sliderId = $atts['id'];
	...
	foreach($slides as $slide){
		...
		(isset($slide['overlay']) && $slide['overlay'] == 'on') ? $overlay = 'overlay dark' : $overlay = '';
		(isset($slide['overlay']) && $slide['overlay'] == 'on') ? $style = 'style="background: url('.$image.'); background-repeat: no-repeat; background-size: cover; background-position: center;"' : $overlay = '';
		...
		if($visibility != 'off'){
			...
			$html .= '<div class="slide-inner '.$overlay.'" '.$style.'>';

			if($bg_link){
				$html .= '<a href="'.$bg_link.'" class="fancybox" rel="slide">';
			}
			if($image){
			...
			if($button_text){
				$html .= '<a class="button btn btn-secondry slider_button" href="'.$button_link.'">'.$button_text.'</a>';
			}
			$html .='</div>';
			$html .= '</div></div>';
